...
set(handles.Plot_Results_button,'Enable','on');
...
if ~iscell(FileName)
    if FileName ~= 0
        ...
        if ~isempty(pat)
            if strcmp(FileName(end-3:end),'.pat')
                s   = FileName(1:end-4);
            end
            pat_list_entries = get(handles.Pattern_List, 'String');
            p   = numel(pat_list_entries)+1 ;
...
for l = 1:handles.maxROI
    if handles.ROIlist(l,handles.pulse) == 1
        ...
        if sum(ind)>0
            ...
            s   = [0; s(:); 0]./max(s);
            plot(ts,interp1(lam, s, ts,'pchip'),'-','Color',handles.ROIcolor(l,:));
        end
    end
end
hold off
...
% --- Executes on button press in Plot_Results_button.
function Plot_Results_button_Callback(hObject, eventdata, handles)
% hObject    handle to Plot_Results_button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
im = Plot_UnmixingResults();
handles.im =im;
guidata(hObject, handles);
