...
%      PATTERN_FIT, by itself, creates a new PATTERN_FIT or raises the existing
%      singleton*.
%
...
% Last Modified by GUIDE v2.5 01-Sep-2011 17:04:03
...
set(handles.popupmenu6, 'String',{'Lifetime', 'FLIM FRET const Int. ratio', 'FLIM FRET const Ampl. ratio'})
...
set(handles.edit1,'String', '0.026');
set(handles.edit2,'String', '6.1');
...
axis([0 floor(handles.taush(end)) -3 0.1]);
...
tic;
[setparams, Ssq, CNT] = LMFsolve(f,setparams);
toc;
...
% --- Executes on button press in getIV.
...
params = [(mesu(I1)),tau1,(mesu(I2)),tau2];
...
if get(handles.varycheck, 'Value') == 1
    ...
    for n = 1:no+1
        ...
        for k = 1:par_num_PIE
            ...
            for i = 1:nch
                irf = handles.IRF(i,:,par_num_PIE).*binw';
                params = [A1,tau1(n),FRETAmp(n), tau2(n)];
                C1 = Pattern_Faltung(params,irf,tau,t,ind);
                C1 = M(i).*C1/sum(C1,2);
                C2 = cat(1,C2,C1);
            end
...
[FileName,PathName] = uiputfile({'*.pat','Pattern';'*.vpat','variable Pattern';...
...
C(C<=1e-3) = 1e-3; % einziger Unterschied zu Pattern_Faltung
