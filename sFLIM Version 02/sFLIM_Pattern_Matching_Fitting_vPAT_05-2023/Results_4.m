...
handles.channel = repmat(lambda,[handles.par_num_PIE 1])> 0;

% handles.channel = repmat(lambda,[handles.par_num_PIE 1]>tmp); %only
% channels with wavelength greater than excitation will be used
...
[FileName,PathName] = uigetfile({'*.pat','Pattern';'*.vpat','variable Pattern';'*.*','All Files'}, ...
         'Load pattern',[handles.pathname 'new.pat'],'MultiSelect', 'off');
    
if ~iscell(FileName)
    if FileName ~= 0 
       pat = importdata([PathName FileName], 'pat');
       if iscell(pat) && strcmp(FileName(end-4:end),'.vpat')
       match = 1; %for variable pattern (pattern family)  
       else match = 2; %for static pattern  
       end
    end  
end


  if match == 2 %for static pattern 
    if ~iscell(FileName)
     if FileName ~= 0 
        if ~isempty(pat)         
 ...
 if match == 1 %load variable pattern 
  ...
  if ~isempty(pat)  
        ...
        load([handles.pathname handles.timname], 'stim')
        tmp = [size(stim,3) size(stim,4) size(stim,5)];
        while (tmp(end)==1)
            tmp(end) = [];
        end
        ...
        if prod(1.*(size(pat) == tmp))
            ...
            set(handles.Pattern_List,'Value',1);
            set(handles.Pattern_List,'String',pat_list_entries);
        elseif (size(stim,3)==size(pat,1))&&(size(stim,5)==size(pat,3))
            pat_list_entries(p) = {s};
            if size(stim,4)<size(pat,2)
                pat = pat(:,1:size(stim,4),:);
                handles.pat(p,:,:,:) = pat./sum(sum(sum(pat)));                            
            else
                handles.pat(p,:,:,:) = zeros(size(stim,3),size(stim,4),size(stim,5)); 
                handles.pat(p,:,1:size(pat,2),:) = pat./sum(sum(sum(pat)));
            end
            set(handles.Pattern_List,'Value',1);
            set(handles.Pattern_List,'String',pat_list_entries);
...
if numel(index_selected) >= 1
 ...
 if match == 1 %&& exist('k','var') % Linear unmixing with one variable pattern (fitting)
    binw   = handles.head.tauw./handles.head.tauw(1);
    DC     = handles.DC;
    aAP    = 1 + handles.AP; 
...
%disp (stim);
    ...
    %BG = BG - (BG - repmat(DC, [numpix 1 1 num_PIE]))./repmat(aAP, [numpix 1 1 num_PIE]);
    %BG = repmat(BG./sum(binw), [1 1 nbin 1]);
    %BG = BG.*permute(repmat(binw(:), [1 numpix nch num_PIE]), [2 3 1 4]);
    
    %stim = stim - BG;

    save('Pattern.mat', 'pat', 'list', 'params');
    
    s = sprintf('Set_PatternOptionsVar(\''file\'',\''Pattern.mat\'',\''%s\'', patfam, stim);',...
...
% Fitting proceedure on all cores of CPU (workers) with Least Squares
        re  = LeastSquares(M,tmp,numpix,num_vpat,num_pat,vParam,vParamDist,final);
        %get the result data from LeastSquares.m
        vParamDist = getappdata(0,'vParamDist');
        final = getappdata(0,'final');
        
% Save and read results for testing display etc.        

%save ('A1.mat', 'vParamDist');
%save ('A2.mat', 'final'); 
% example1 = matfile('A1.mat');
% vParamDist = example1.vParamDist;
% example2 = matfile('A2.mat');
% final = example2.final;

%         disp(size(res));
%         disp(size(final));
%         disp(size(vParamDist));
 ...
 if match == 0   %Standard linear unmixing without variable pattern
  %end  
...
%         disp(size(otmp));
        ...
        res(:,:,num_pat+1) =  otmp - sum(res,3); %Residuals
...
results.match = handles.match;
results.figures = handles.figPA;
if  isfield(handles, 'vParam')% && ishandle(handles.vParam)
   results.vParam = handles.vParam;
 else
   results.vParam = [];
end
if  isfield(handles, 'vParamDist')% && ishandle(handles.vParamDist)
   results.vParamDist = handles.vParamDist;
 else
   results.vParamDist = [];
end
...
%disp(size(handles.M_X));
%disp(size(handles.M_Y));
